...
-- 리터럴
-- 임으로 정한 문자열을 SELECT절에 사용하면 테이블에 존재하는 데이터처럼 사용 가능
...
-- DISTINCT : 컬럼에 포함된 중복 값을 한 번씩만 표기하고자 할 때 사용, SELECT절에 딱 한 번만 쓸 수 있음
-- EMPLOYEE 테이블에서 직원의 직급 코드 조회
SELECT JOB_CODE
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 직원의 직급코드를 중복제거하여 조회
SELECT DISTINCT JOB_CODE
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 부서코드와 직급코드를 중복제거 하여 조회
SELECT DISTINCT DEPT_CODE, JOB_CODE   -- 두 개가 하나로 묶여서 중복제거 됨
FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE (DEPT_CODE = 'D9' OR DEPT_CODE ='D5') AND HIRE_DATE < '02/01/01';     -- AND가 OR보다 우선 순위가 높음
...
SELECT EMP_ID,EMP_NAME,EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '___*_%' ESCAPE '*';       -- ESCAPE 뒤에 있는 것은 문자라고 알려줌 
...
SELECT *
FROM EMPLOYEE
WHERE EMAIL LIKE '____1_%' ESCAPE '1' AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6') AND HIRE_DATE BETWEEN '90/01/01' AND 
...
-- EMPLOYEE 테이블에서 부서코드가 D9이거나 D6인 사원의 이름, 부서코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
